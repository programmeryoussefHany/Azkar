...
import { ArrowLeft, Moon, Clock, CheckCircle } from 'lucide-react'
...
export default function EveningPage() {
  ...
  useEffect(() => {
    ...
    const savedCompleted = localStorage.getItem('azkar-completed-evening')
    if (savedCompleted) {
      setCompletedAdhkar(JSON.parse(savedCompleted))
    }

    // Load dhikr counts from localStorage
    const savedCounts = localStorage.getItem('azkar-counts-evening')
    if (savedCounts) {
  ...
  const handleComplete = (id: string) => {
    const dhikr = eveningAdhkar.find(d => d.id === id)
    ...
    if (completedAdhkar.includes(id)) {
      ...
      localStorage.setItem('azkar-completed-evening', JSON.stringify(newCompleted))
  ...
  const handleDhikrCount = (id: string, count: number) => {
    ...
    localStorage.setItem('azkar-counts-evening', JSON.stringify(newCounts))
  ...
  const resetProgress = () => {
    setCompletedAdhkar([])
    setDhikrCounts({})
    localStorage.removeItem('azkar-completed-evening')
    localStorage.removeItem('azkar-counts-evening')
  }

  // Calculate total counts and completed counts
  const totalRequiredCount = eveningAdhkar.reduce((sum, dhikr) => sum + (dhikr.count || 1), 0)
  const completedCount = eveningAdhkar.reduce((sum, dhikr) => {
  ...
  const getEveningGreeting = () => {
    ...
    if (hour >= 17 && hour < 21) {
      return 'مساء الخير - وقت أذكار المساء'
    } else if (hour >= 21 || hour < 5) {
      return 'ليلة مباركة - وقت أذكار المساء'
    } else {
      return 'أذكار المساء - للقراءة من العصر حتى المغرب'
    }
  }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <div className="bg-gradient-to-r from-purple-500 to-indigo-600 rounded-xl p-6 text-white">
          ...
          {/* Progress Bar */}
          <div className="mt-4">
            <div className="bg-purple-300/30 rounded-full h-2">
              <div 
          ...
          {completionPercentage === 100 && (
            <div className="mt-3 flex items-center text-purple-100">
              <CheckCircle className="w-5 h-5 ml-2" />
              <span className="arabic-text">مبارك! لقد أكملت جميع أذكار المساء</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-6">
        {eveningAdhkar.map((dhikr, index) => (
          <div key={dhikr.id} className="relative">
            {/* Number Badge */}
            <div className="absolute -right-3 -top-3 w-8 h-8 bg-purple-500 text-white rounded-full flex items-center justify-center text-sm font-bold z-10">
              {index + 1}
            </div>
      ...
      <div className="mt-12 p-6 bg-white dark:bg-gray-800 rounded-lg shadow-md text-center">
        <p className="text-gray-600 dark:text-gray-300 arabic-text mb-2">
          &ldquo;وَسَبِّحْ بِحَمْدِ رَبِّكَ قَبْلَ طُلُوعِ الشَّمْسِ وَقَبْلَ غُرُوبِهَا&rdquo;
        </p>
